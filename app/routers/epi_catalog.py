...
async def list_catalog(
    ...
    total = query.count()
    items = (
        query.order_by(EpiCatalog.ativo.desc(), func.upper(EpiCatalog.nome))
...
@router.post("")
async def create_catalog_entry(data: EpiCatalogCreate, db: Session = Depends(get_db)):
    ...
    epi = EpiCatalog(nome=data.nome.strip(), ativo=True)
    for s in data.sizes:
...
@router.put("/{epi_id}")
async def update_catalog_entry(epi_id: int, data: EpiCatalogCreate, db: Session = Depends(get_db)):
    epi = db.query(EpiCatalog).options(joinedload(EpiCatalog.sizes)).filter(EpiCatalog.id == epi_id).first()
    if not epi:
        return JSONResponse(status_code=404, content={"status": "error", "message": "EPI não encontrado"})
...
async def deactivate_catalog_entry(epi_id: int, db: Session = Depends(get_db)):
...
async def reactivate_catalog_entry(epi_id: int, db: Session = Depends(get_db)):
    epi = db.query(EpiCatalog).options(joinedload(EpiCatalog.sizes)).filter(EpiCatalog.id == epi_id).first()
    if not epi:
        return JSONResponse(status_code=404, content={"status": "error", "message": "EPI não encontrado"})
